...
def sample_target_mask(im, mask, target_bb, search_area_factor, output_sz=None):
    """ Extracts a square crop centered at target_bb box, of area search_area_factor^2 times target_bb area
    ...
    x1 = round(x + 0.5*(w-1) - (crop_sz-1)*0.5)
    x2 = x1 + crop_sz - 1

    y1 = round(y + 0.5*(h-1) - (crop_sz-1) * 0.5)
    y2 = y1 + crop_sz - 1
    ...
    # Pad
    im_crop_padded = cv.copyMakeBorder(im_crop, y1_pad, y2_pad, x1_pad, x2_pad, cv.BORDER_REPLICATE)
    mask_crop_padded = cv.copyMakeBorder(mask_crop, y1_pad, y2_pad, x1_pad, x2_pad, cv.BORDER_REPLICATE)
    # mask_crop_padded = np.pad(mask_crop, ((y1_pad, y2_pad), (x1_pad, x2_pad)), 'edge')  # same as cv.copyMakeBorder

    if output_sz is not None:
        resize_factor = output_sz / crop_sz
        return cv.resize(im_crop_padded, (output_sz, output_sz)), \
            cv.resize(mask_crop_padded, (output_sz, output_sz)), resize_factor
    else:
        return im_crop_padded, mask_crop_padded, 1.0
...
def transform_image_to_crop(box_in: torch.Tensor, box_extract: torch.Tensor, resize_factor: float,
    ...
    """
    box_extract_center = box_extract[0:2] + 0.5*(box_extract[2:4]-1)

    box_in_center = box_in[0:2] + 0.5*(box_in[2:4]-1)

    box_out_center = (crop_sz-1)/2 + (box_in_center - box_extract_center)*resize_factor
    box_out_wh = box_in[2:4]*resize_factor

    box_out = torch.cat((box_out_center - 0.5*(box_out_wh-1), box_out_wh))
    return box_out


def transform_image_to_crop_v2(box_in: torch.Tensor, box_extract: torch.Tensor, resize_factor: float,
                               crop_sz: torch.Tensor, output_sz) -> torch.Tensor:
    """ Transform the box co-ordinates from the original image co-ordinates to the co-ordinates of the cropped image
        Difference between v1 and v2 is that v2 adds a restriction on bbox, preventing is from getting
    ...
    box_out = torch.cat((box_out_center - 0.5*(box_out_wh-1), box_out_wh))
    box_out[box_out < 0] = 0
    box_out[box_out > output_sz-1] = output_sz-1
...
def jittered_center_crop_mask(frames, box_extract, mask, box_gt, search_area_factor, output_sz):
    """ For each frame in frames, extracts a square crop centered at box_extract, of area search_area_factor^2
    times box_extract area. The extracted crops are then resized to output_sz. Then do the same to mask.
    Further, the co-ordinates of the box box_gt are transformed to the image crop co-ordinates

    args:
        frames - list of frames
        box_extract - list of boxes of same length as frames. The crops are extracted using anno_extract
        mask - list of masks of each frame
        box_gt - list of boxes of same length as frames. The co-ordinates of these boxes are transformed from
                    image co-ordinates to the crop co-ordinates
        search_area_factor - The area of the extracted crop is search_area_factor^2 times box_extract area
        output_sz - The size to which the extracted crops are resized
    ...
    returns:
        ...
        list - box_gt location in the crop co-ordinates
        """
    crops_resize_factors = [sample_target_mask(f, m, a, search_area_factor, output_sz)
                            for f, m, a in zip(frames, mask, box_extract)]

    frames_crop, masks_crop, resize_factors = zip(*crops_resize_factors)

    crop_sz = torch.Tensor([output_sz, output_sz])

    # find the bb location in the crop
    box_crop = [transform_image_to_crop_v2(a_gt, a_ex, rf, crop_sz, output_sz)
                for a_gt, a_ex, rf in zip(box_gt, box_extract, resize_factors)]

    return frames_crop, masks_crop, box_crop
...
def perturb_box_v2(box, min_iou=0.5, sigma_factor=0.1):
    ...
    # multiple tries to ensure that the perturbed box has iou > min_iou with the input box
    for i_ in range(100):
        c_x = box[0] + 0.5 * (box[2]-1)
        c_y = box[1] + 0.5 * (box[3]-1)
        c_x_per = gauss(c_x, perturb_factor[0])
        c_y_per = gauss(c_y, perturb_factor[1])
        ...
        box_per = torch.Tensor([c_x_per - 0.5*(w_per-1), c_y_per - 0.5*(h_per-1), w_per, h_per]).round()
